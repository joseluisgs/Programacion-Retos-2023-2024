...
class HeroesMarvelRepositoryTest {

    private val repo = HeroesMarvelRepository()
    ...
    fun getAllHeroes() {
        ...
        assertAll(
            ...
            { assertEquals("Test01Marvel", heroes[0].nombre) },
            { assertEquals(120, heroes[1].altura) },
            { assertEquals(30, heroes[0].edad) },
            { assertEquals("Test02Marvel", heroes[1].notas) }
        )
    }
    ...
    fun getHeroeById() {
        ...
        assertAll(
            ...
            { assertEquals("Test01Marvel", heroe?.nombre) }
        )
    }
    ...
    fun repositoryResizedInSave() {
        ...
        assertAll(
            ...
            { assertEquals("Test01Marvel", res[0].nombre) },
            { assertEquals("Test01Marvel", res[0].alias) },
            { assertEquals(100, res[0].altura) },
            { assertEquals(30, res[0].edad) },
            { assertEquals("Test01Marvel", res[0].notas) },
            { assertEquals(2, res[1].id) },
            { assertEquals("Test02Marvel", res[1].nombre) },
            { assertEquals("Test02Marvel", res[1].alias) },
            { assertEquals(120, res[1].altura) },
            { assertEquals(40, res[1].edad) },
            { assertEquals("Test02Marvel", res[1].notas) },
